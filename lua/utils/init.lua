...
local function r_inspect_settings(structure, limit, separator)
  ...
  if type(structure) == "function" then
    ...
    io.write("-- ufovim", separator:sub(2), " = function ()\n")
  else
    io.write("ufovim", separator:sub(2), " = ", tostring(structure), "\n")
  end
  return limit - 1
end
...
function utils.generate_settings()
  ...
  -- write all `ufovim` related settings to `lv-settings.lua` file
  r_inspect_settings(ufovim, 10000, ".")
...
function utils.toggle_autoformat()
  ...
  if not ufovim.format_on_save then
    vim.cmd [[
...
function utils.reload_lv_config()
  ...
  vim.cmd("source " .. utils.join_paths(get_runtime_dir(), "ufovim", "lua", "plugins.lua"))
  local plugins = require "plugins"
  utils.toggle_autoformat()
  local plugin_loader = require "plugin-loader"
  plugin_loader:cache_reset()
  plugin_loader:load { plugins, ufovim.plugins }
