...
local function lsp_highlight_document(client)
  if ufovim.lsp.document_highlight == false then
...
local function select_default_formater(client)
  ...
  for _, filetype in ipairs(client_filetypes) do
    if not vim.tbl_isempty(ufovim.lang[filetype].formatters) then
...
function M.common_on_init(client, bufnr)
  if ufovim.lsp.on_init_callback then
    ufovim.lsp.on_init_callback(client, bufnr)
...
function M.common_on_attach(client, bufnr)
  if ufovim.lsp.on_attach_callback then
    ufovim.lsp.on_attach_callback(client, bufnr)
...
function M.setup()
  ...
  for _, sign in ipairs(ufovim.lsp.diagnostics.signs.values) do
