...
function M.setup()
  local config = { -- your config
    virtual_text = ufovim.lsp.diagnostics.virtual_text,
    signs = ufovim.lsp.diagnostics.signs,
    underline = ufovim.lsp.diagnostics.underline,
    update_in_insert = ufovim.lsp.diagnostics.update_in_insert,
    severity_sort = ufovim.lsp.diagnostics.severity_sort,
  }
  if vim.fn.has "nvim-0.5.1" > 0 then
    vim.lsp.handlers["textDocument/publishDiagnostics"] = function(_, result, ctx, _)
      ...
      if ok then
        ...
        for i, sign in ipairs(ufovim.lsp.diagnostics.signs.values) do
  ...
  vim.lsp.handlers["textDocument/hover"] = vim.lsp.with(vim.lsp.handlers.hover, {
    border = ufovim.lsp.popup_border,
  })
