...
ufovim.lsp.templates_dir = join_paths(get_runtime_dir(), "ufovim", "tests", "artifacts")
...
a.describe("lsp workflow", function()
  ...
  a.it("shoud be able to delete ftplugin templates", function()
    if utils.is_directory(ufovim.lsp.templates_dir) then
      assert.equal(vim.fn.delete(ufovim.lsp.templates_dir, "rf"), 0)
    end
    assert.False(utils.is_directory(ufovim.lsp.templates_dir))
  end)
  ...
  a.it("shoud be able to generate ftplugin templates", function()
    ...
    vim.schedule(function()
      assert.True(utils.is_directory(ufovim.lsp.templates_dir))
    end)
  end)

  a.it("shoud not attempt to re-generate ftplugin templates", function()
    ufovim.log.level = "debug"
  ...
  a.it("shoud ignore all javascript servers except tsserver and tailwindcss when generating templates", function()
    ...
    local ts_template = utils.join_paths(ufovim.lsp.templates_dir, "typescript.lua")
  ...
  a.it("shoud not include blacklisted servers in the generated templates", function()
    ...
    for _, file in ipairs(vim.fn.glob(ufovim.lsp.templates_dir .. "/*.lua", 1, 1)) do
